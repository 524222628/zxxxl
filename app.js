...
function transportDiagram(block) {
  ...
  return `<details class="transit-route"><summary class="transit-route-summary"><span class="transit-route-heading"><span>本次交通线路</span><strong>${escapeHtml(block.transport || '交通方式待核验')}</strong></span><span class="route-track">${nodes}</span><span class="transit-route-affordance" aria-hidden="true"></span></summary><div class="transit-route-detail"><ol class="ride-steps"><li><b>从「${escapeHtml(stops[0])}」进站</b><span>寻找「${escapeHtml(block.transport || '铁路 / 地铁')}」标识，用 ICOCA / Suica 或单程票刷闸。先看电子屏确认列车终点和发车时间。</span></li><li><b>上车后盯住站名</b><span>车内屏幕、车门上方路线图和站牌都会显示下一站。不要只凭颜色判断方向；听到站名后再准备下车。</span></li>${transferSteps}<li><b>抵达「${escapeHtml(stops.at(-1))}」</b><span>出闸前先打开地图确认出口与步行方向；若时间紧，优先按站内「出口」标识离站，不在闸机附近整理行李。</span></li></ol><p><b>${escapeHtml(block.start)} - ${escapeHtml(block.end)}</b> 是本段预留时间。此图为行程示意，站台、终点站显示、换乘与班次以当天电子屏和运营方信息为准。</p></div></details>`;
}
